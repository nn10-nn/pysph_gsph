"""1D SRHD Riemann Problem 2 (Ferrer-Sanchez 2024, Eq. 27).

Problem setup (physical window):
- x in [0, 1], discontinuity at x=0.5
- t_end = 0.4
- gamma = 5/3
- rho_L = 1.0, u_L = 0.5, p_L = 0.1
- rho_R = 1.0, u_R = 0.0, p_R = 1.0
"""
...
class RiemannProblem2Rel(SodShockTubeRel):
    def __init__(self, *args, **kwargs):
        super(RiemannProblem2Rel, self).__init__(*args, **kwargs)
    ...
    def consume_user_options(self):
        ...
        if self.rank == 0:
            print(
                "[DEBUG:resolution] using nl=%d, hdx=%.3f (Problem 2)"
                % (self.nl, self.hdx)
            )
    ...
    def create_particles(self):
        ...
        if self.rank == 0 and (not self._dbg_init_done):
            print("\n[DEBUG:init] Problem-2 target states:")
            print("  Left : rho=1.0, u=0.5, p=0.1")
            print("  Right: rho=1.0, u=0.0, p=1.0")
